...
stasisStart(StasisStart event) {
  ...
  if (!dialed) {
    ...
    resp.then((err) async {
      ...
      // channel.on('callEndedUnanswered', (callback) {
      //   originate(channel, false, playback, false);
      // });
      // bool result = await originate(channel, false, playback, false);
      // while (result) {
      //   await Future.delayed(Duration(milliseconds: 5000));
      //   result = await originate(channel, false, playback, false);
      // }

      // originate(channel, false, playback, false).then((onValue) {
      //   if (onValue == false) {
      //     print("originate result: $onValue");
      originate(channel, false, playback, false);
      //   }
      // });

      //       bool result = await futureBool;
      // while (!result) {
      //   await Future.delayed(Duration(milliseconds: 100));
      //   result = await futureBool;
      // }
...
Future<bool> originate(Channel channel, bool incomingStasisEnd,
    Playback playback, bool localDialedChannelDestroyed) async {
  ...
  dialed.on('ChannelStateChange', (event) async {
    ...
    ChannelStateChange csc = event as ChannelStateChange;
  ...
  channel.on('StasisEnd', (event) {
    ...
    StasisEnd cse = event as StasisEnd;
    ...
    if (dsbClient != null) {
      if (voiceRecords[channel.id] != null) {
        voiceRecords[channel.id]!.duration_number = cse.timestamp.toString();
  ...
  dialed.on('ChannelDestroyed', (event) {
    ...
    if (succeeded) {
      ChannelDestroyed ed = event as ChannelDestroyed;
      //Channel cd = ed.channel as Channel;
      //Channel ch = event.sender as Channel;
      ...
      callTimers[channel.id] = setTimeout(() {
        ...
        originate(
            channel, incomingStasisEnd, playback, localDialedChannelDestroyed);
      });

      //channel.emit('callEndedUnanswered', channel);
      return true;
      // }
    }
  });
...
void joinMixingBridge(Channel channel, Channel dialed, Bridge mixingBridge) {
  dialed.on('Dialed StasisEnd', (event) {
    // print("Event name: ${event.eventName}");
    // print("Event data: ${event.eventData}");
    // print("Event sender: ${event.sender}");
    // print("");
...
void call_center_bridge(List<String> args) async {
  ...
  client.on("StasisStart", (event) {
    ...
    stasisStart(event as StasisStart);
  });
      ...
      case "StasisStart":
        {
          ...
          //       .emit(data['type'], stasisStartEvent);
          // } else {
          ...
          channel.stasisStart(stasisStartEvent);
      ...
      case 'StasisEnd':
        {
          ...
          if (client.statisChannels[channel.id] != null) {
            ...
            print("");
            client.statisChannels[channel.id]!
                .emit('StasisEnd', stasisEndEvent);
          } else {
            client.statisChannels[channel.id] = channel;
            client.statisChannels[channel.id]!
                .emit(data['type'], stasisEndEvent);
          }

          client.emit(data['type'], stasisEndEvent);
        }
      case 'ChannelDestroyed':
        {
          ...
          if (client.statisChannels[channel.id] != null) {
            client.statisChannels[channel.id]!
                .emit(data['type'], channelDestroyedEvent);
          } else {
            client.statisChannels[channel.id] = channel;
            client.statisChannels[channel.id]!
                .emit(data['type'], channelDestroyedEvent);
          }
          channel.channelDestroyed(channelDestroyedEvent);

          // client.emit(data['type'], channel, channelDestroyedEvent);

          //client.channelDestroyed(channelDestroyedEvent);

          client.emit(data['type'], channelDestroyedEvent);
        }
      case 'ChannelStateChange':
        {
          ...
          client.emit(data['type'], channelStateChangeEvent);
      ...
      case 'ChannelDtmfReceived':
        {
          ...
          client.emit(data['type'], channelDtmfReceivedEvent);
        }
